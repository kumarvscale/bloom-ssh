...
from bloom import DEBUG_MODE, debug_print
from bloom.transcript_utils import add_transcript_event, generate_id, get_transcript_format, init_transcript_metadata
from bloom.utils import DEFAULT_TEMPERATURE_DETERMINISTIC, litellm_chat, parse_message
...
class ConversationOrchestrator:
    ...
    def setup(
        ...
        skip_target_system_prompt: bool = False,
    ) -> ConversationOrchestrator:
        # Check if we should skip generating a system prompt for the target
        if skip_target_system_prompt:
            # Use empty system prompt - target model receives no system context
            target_system_prompt = ""
            system_prompt_content = "[System prompt generation skipped - blank system prompt]"
            system_prompt_reasoning = ""
            debug_print("⚠️ Skipping target system prompt generation (blank system prompt)")
        else:
