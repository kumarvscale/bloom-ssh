...
# Custom LiteLLM Proxy Configuration
# Set these environment variables to use a custom LiteLLM proxy:
...
# If not set, uses direct provider API calls via standard LiteLLM routing


def get_litellm_proxy_config() -> tuple[Optional[str], Optional[str]]:
    """
    Get LiteLLM proxy configuration from environment variables.
    
    Returns:
        Tuple of (api_base, api_key) or (None, None) if not configured.
    """
    api_key = os.getenv("LITELLM_API_KEY")
    # Support both LITELLM_BASE_URL and LLM_URL for flexibility
    api_base = os.getenv("LITELLM_BASE_URL") or os.getenv("LLM_URL")
    
    # Strip /chat/completions suffix if present (LiteLLM adds it automatically)
...
def is_using_litellm_proxy() -> bool:
    """Check if a custom LiteLLM proxy is configured."""
    api_base, api_key = get_litellm_proxy_config()
    return api_base is not None and api_key is not None


def transform_model_for_proxy(model_id: str) -> str:
    """
    Transform model ID for use with LiteLLM proxy.
    
    When using a LiteLLM proxy, models should be passed with the 'openai/' prefix
    since the proxy handles routing to the actual provider.
    
    Args:
        model_id: Original LiteLLM model ID (e.g., "anthropic/claude-sonnet-4")
        
    Returns:
        Model ID suitable for the proxy (e.g., "openai/claude-sonnet-4")
    """
    # If already using openai/ prefix, return as-is
...
def litellm_chat(
    ...
    **kwargs,
    ...
    Supports two modes:
    1. Direct provider API calls (default) - Uses standard LiteLLM routing
    2. Custom LiteLLM Proxy - When LITELLM_API_KEY and LITELLM_BASE_URL are set
    
    Environment Variables for Proxy Mode:
        LITELLM_API_KEY: API key for the LiteLLM proxy
        LITELLM_BASE_URL: Base URL of the proxy (e.g., https://litellm.example.com)
        LLM_URL: Alternative to LITELLM_BASE_URL
    """
    # litellm._turn_on_debug()
    
    # Check if using custom LiteLLM proxy
    api_base, api_key = get_litellm_proxy_config()
    using_proxy = api_base is not None and api_key is not None
    
    # Transform model ID for proxy if needed
    effective_model_id = model_id
    if using_proxy:
        effective_model_id = transform_model_for_proxy(model_id)
    ...
    # Add proxy configuration if using custom LiteLLM proxy
    if using_proxy:
    ...
    # Skip reasoning check when using proxy (proxy handles model capabilities)
    supports_reasoning = False
    if not using_proxy:
        try:
            supports_reasoning = litellm.supports_reasoning(model=model_id)
        except Exception:
            supports_reasoning = False
    
    if reasoning_effort != "none" and tool_choice != "required" and (using_proxy or supports_reasoning):
...
def validate_api_keys(config: Dict[str, Any]) -> tuple[bool, Optional[str]]:
    ...
    When using a LiteLLM proxy (LITELLM_API_KEY and LITELLM_BASE_URL are set),
    only validates proxy credentials instead of individual provider keys.
    ...
    """
    # Check if using LiteLLM proxy mode
    if is_using_litellm_proxy():
        # In proxy mode, we only need the proxy credentials
        # which are already verified by is_using_litellm_proxy()
        return True, None
    
    # Check if proxy is partially configured (missing one of the required vars)
    litellm_api_key = os.getenv("LITELLM_API_KEY")
    litellm_base_url = os.getenv("LITELLM_BASE_URL") or os.getenv("LLM_URL")
    
    if litellm_api_key and not litellm_base_url:
        error_lines = [
            "=" * 60,
            "❌ INCOMPLETE LITELLM PROXY CONFIGURATION",
            "=" * 60,
            "",
            "  LITELLM_API_KEY is set but LITELLM_BASE_URL is missing.",
            "",
            "  To use LiteLLM proxy mode, set both:",
            "    LITELLM_API_KEY=your_api_key",
            "    LITELLM_BASE_URL=https://your-litellm-proxy.com",
            "",
            "  Or remove LITELLM_API_KEY to use direct provider API calls.",
            "=" * 60,
        ]
        return False, "\n".join(error_lines)
    ...
    if litellm_base_url and not litellm_api_key:
        error_lines = [
            ...
            "=" * 60,
            "",
            "  LITELLM_BASE_URL is set but LITELLM_API_KEY is missing.",
            "",
            "  To use LiteLLM proxy mode, set both:",
            "    LITELLM_API_KEY=your_api_key",
            "    LITELLM_BASE_URL=https://your-litellm-proxy.com",
            "",
            "  Or remove LITELLM_BASE_URL to use direct provider API calls.",
            "=" * 60,
        ]
        return False, "\n".join(error_lines)
    
    # Standard validation for direct provider API calls
    ...
    if missing_keys:
        ...
        error_lines.append("")
        error_lines.append("💡 Alternatively, configure LiteLLM proxy mode:")
        error_lines.append("   LITELLM_API_KEY=your_proxy_api_key")
        error_lines.append("   LITELLM_BASE_URL=https://your-litellm-proxy.com")
